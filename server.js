...
app.get('/tgp/:tgp_id/history', (req, res) => {
  let sql = "SELECT form_id, date_format(update_time, '%Y년 %m월 %d일 %H시 %i분 %s초') AS update_time FROM FORM WHERE tgp_id = " + req.params.tgp_id + " and delete_time IS NULL ORDER BY update_time DESC LIMIT 100 OFFSET 0";
...
app.get('/tgp/:tgp_id/step1', (req, res) => {
  // 가장 최근 히스토리 조회
  let sql = "SELECT form_id, account, department, solution, amount, date_format(closingdate, '%Y-%m-%d') AS closingdate FROM FORM WHERE tgp_id = " + req.params.tgp_id + " ORDER BY form_id DESC LIMIT 1";
...
// TGP STEP1 조회 - 내역 불러온 경우
app.get('/tgp/:tgp_id/step1/:form_id', (req, res) => {
  // 가장 최근 히스토리 조회
  let sql = "SELECT form_id, account, department, solution, amount, date_format(closingdate, '%Y-%m-%d') AS closingdate FROM FORM WHERE tgp_id = " + req.params.tgp_id + " AND form_id = " + req.params.form_id;

  console.log("STEP1 조회(내역): " + sql);
  connection.query(sql, (err, rows, fields) => {
    res.send(rows);
  });
});
...
// TGP STEP1 저장 - 내역 불러온 경우
app.post('/tgp/:tgp_id/step1/:form_id', (req, res) => {
  let tgp_id = req.params.tgp_id;
  ...
  // 복사 - FORM           
  sql = "INSERT INTO FORM (`tgp_id`, `position1`, `position1_sign`, `position2`, `position2_sign`, `position3`, `position3_sign`, `competition1_name`, `competition1_name_sign`, `competition1_type`, `competition1_type_sign`, `competition2_name`, `competition2_name_sign`, `competition2_type`, `competition2_type_sign`, `make_time`) SELECT `tgp_id`, `position1`, `position1_sign`, `position2`, `position2_sign`, `position3`, `position3_sign`, `competition1_name`, `competition1_name_sign`, `competition1_type`, `competition1_type_sign`, `competition2_name`, `competition2_name_sign`, `competition2_type`, `competition2_type_sign`, `make_time` FROM FORM WHERE form_id = " + form_id;

  console.log("STEP1 저장(복사): " + sql);
  connection.query(sql);

  // 방금 인서트한 form_id 구하기
  sql = "SELECT form_id FROM FORM WHERE tgp_id = " + tgp_id + " ORDER BY form_id DESC LIMIT 1;";
  ...
  connection.query(sql, (err, rows, fields) => {
    ...
    // 업데이트 - FORM
    sql = "UPDATE FORM SET account = ?, department = ?, solution = ?, amount = ?, closingdate = ?, update_time = now(), make_time = now() WHERE form_id = " + new_form_id;
