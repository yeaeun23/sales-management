...
app.get('/api/tgp/:tgp_id/:form_id', (req, res) => {
  let sql = "SELECT account, department, solution, amount, date_format(closingdate, '%Y-%m-%d') AS closingdate FROM FORM WHERE form_id = ? and tgp_id = ?";
...
app.get('/api/tgp/:tgp_id/:form_id/FORM', (req, res) => {
  let sql = "SELECT * FROM FORM WHERE form_id = ? and tgp_id = ?";
  let params = [req.params.form_id, req.params.tgp_id];
...
// TGP STEP1 저장
app.post('/api/tgp/:tgp_id/STEP1', upload.single('image'), (req, res) => {
  let sql = 'INSERT INTO FORM VALUES (null, ?, ?, ?, ?, ?, ?);';
